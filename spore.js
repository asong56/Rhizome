(()=>{
    ...
    function getOverlay() {
        ...
        (overlay = document.createElement('div')).style.cssText =
    ...
    class RhizomeSpore extends HTMLElement {
        connectedCallback() {
            const t     = THEMES[this.getAttribute('theme')] ?? THEMES.light;
...
.w{display:inline-flex;align-items:center;font-family:-apple-system,BlinkMacSystemFont,"Helvetica Neue",sans-serif;font-size:12px;letter-spacing:.04em;border:1px solid ${t.br};border-radius:20px;background:${t.bg};color:${t.fg};backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);overflow:hidden;user-select:none}
.l{padding:7px 14px 7px 16px;opacity:.5;font-weight:500;white-space:nowrap}
.d{width:1px;background:${t.br};align-self:stretch;flex-shrink:0}
b{all:unset;cursor:pointer;padding:7px 13px;opacity:.5;background:radial-gradient(ellipse at 50% 50%,${t.ink} 0%,transparent 65%);background-size:0 0;background-position:50% 50%;background-repeat:no-repeat;transition:opacity .18s,background-size .5s cubic-bezier(.22,1,.36,1)}
...
<div class="w" role="navigation" aria-label="Rhizome webring">
    <span class="l">${label}</span><i class="d"></i>
    <b data-d="p" title="Previous">
        <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
            <path d="M8.5 2L4 6.5l4.5 4.5" stroke="${t.fg}" stroke-width="1.35" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>
    </b><i class="d"></i>
    <b data-d="r" title="Random">
        <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
            <circle cx="6.5" cy="6.5" r="4" stroke="${t.fg}" stroke-width="1.2"/>
            <circle cx="6.5" cy="6.5" r="1.5" fill="${t.ac}"/>
        </svg>
    </b><i class="d"></i>
    <b data-d="n" title="Next">
        <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
            <path d="M4.5 2L9 6.5 4.5 11" stroke="${t.fg}" stroke-width="1.35" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>
    </b>
</div>`;
